  ...
  if (showPreview) {
    return (
      <div className="min-h-screen bg-slate-100 py-12 px-4 font-sans text-slate-900">
        <div id="brief-document" className="max-w-[800px] mx-auto bg-white shadow-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="bg-slate-900 p-12 text-center border-b-8 border-blue-600">
            ...
            <h1 className="text-white text-3xl font-black tracking-widest uppercase">MLL DIGITAL CONSULTING</h1>
            <p className="text-blue-400 text-[10px] font-bold tracking-[0.5em] uppercase mt-2">Systems Architecture & IT Strategy</p>
          </div>
          ...
          <div className="p-12 lg:p-20 space-y-12">
            <div>
              ...
              <p className="text-slate-400 text-xs mt-2 font-mono uppercase">Ref: MLL-OUTREACH-{new Date().getFullYear()}</p>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 gap-6">
                {findings.map((f, i) => (
                  <div key={i} className="bg-slate-50 p-8 rounded-xl border border-slate-200 relative overflow-hidden group">
                    <Zap className="absolute -right-4 -bottom-4 h-24 w-24 text-blue-600/5 transition-transform group-hover:scale-110" />
                    <div className="relative z-10">
                      <p className="text-[10px] font-black text-blue-600 uppercase mb-4 tracking-tighter">Observation {i + 1}</p>
                      <p className="text-sm font-bold text-slate-500 uppercase tracking-tight">Identified Constraint:</p>
               ...
               <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                 <div>
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">
                      Every project initiated by MLL Digital utilizes AES-256 Bit Data Encryption, POPIA compliant architecture, and high-performance Next.js 14 infrastructure as standard protocol.
                    </p>
                 </div>
                 <div className="text-right">
                    <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-2">Team Signature</p>
                    <p className="font-serif italic text-2xl">MLL Digital Engineering</p>
                 </div>
               </div>
            </div>
            ...
            <div className="text-center pt-10 border-t border-slate-100">
               <p className="text-sm text-slate-500 leading-relaxed max-w-lg mx-auto italic">
                 &quot;If our technical vision aligns with your brand goals, we invite you to view our full portfolio and arrange a briefing to discuss a way forward.&quot;
               </p>
               <a href="https://ml-consulting-iota.vercel.app/" className="inline-block mt-8 text-blue-600 font-bold text-xs uppercase tracking-[0.3em] border-b-2 border-blue-600 pb-1">
                 View Portfolio Site
               </a>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-12 border-b-4 border-slate-900 pb-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-blue-600 font-bold text-[10px] uppercase tracking-[0.3em] mt-1">MLL Digital Consulting // Internal Engine</p>
            </div>
          </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); setShowPreview(true); }} className="space-y-8">
          ...
          <div className="bg-white p-10 rounded-xl border border-slate-200 shadow-sm space-y-8">
            <h3 className="text-xs font-black text-blue-600 uppercase border-l-4 border-blue-600 pl-6 tracking-widest">1.0 Honoring the Target</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <input required placeholder="Client Name" className="border-b-2 p-3 outline-none focus:border-blue-600 font-bold text-slate-900" onChange={e => setClient({...client, name: e.target.value})} />
              <input required placeholder="Target Website URL" className="border-b-2 p-3 outline-none focus:border-blue-600 font-bold text-blue-600" onChange={e => setClient({...client, website: e.target.value})} />
            </div>
          ...
          <div className="bg-white p-10 rounded-xl border border-slate-200 shadow-sm space-y-8">
            <div className="flex justify-between items-center">
              <h3 className="text-xs font-black text-blue-600 uppercase border-l-4 border-blue-600 pl-6 tracking-widest">2.0 Assessment Findings</h3>
            ...
            {findings.map((f, i) => (
              <div key={i} className="p-6 bg-slate-50 border rounded-xl space-y-4 relative">
                <input placeholder="Identify System Bottleneck" className="w-full bg-white border rounded p-3 text-xs font-bold" value={f.issue} onChange={e => { const n = [...findings]; n[i].issue = e.target.value; setFindings(n); }} />
                <input placeholder="Proposed MLL Elevation" className="w-full bg-white border rounded p-3 text-xs font-black text-blue-600" value={f.solution} onChange={e => { const n = [...findings]; n[i].solution = e.target.value; setFindings(n); }} />
                <input placeholder="Direct Business Benefit" className="w-full bg-white border rounded p-3 text-[10px] italic" value={f.benefit} onChange={e => { const n = [...findings]; n[i].benefit = e.target.value; setFindings(n); }} />
          ...
          <button type="submit" className="w-full bg-slate-900 text-white font-black py-8 rounded-xl hover:bg-blue-600 transition-all uppercase tracking-[0.4em] text-xs shadow-2xl flex items-center justify-center gap-4">
            GENERATE STRATEGIC BRIEFING <ArrowRight className="h-4 w-4" />
          </button>
